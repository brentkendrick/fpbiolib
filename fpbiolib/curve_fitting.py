...
def component_pks_fm_model(x, pk_idx, meth, lmfit_option, params):
    ...
    y_model_pks = np.asarray(
        y_model_pks
    )  # convert list to an array, generally takes less memory
...
def gaussian_component_pks(params, x, pk_idx, lmfit=False):
    ...
    for i in range(num_pks):
        ...
        y_fun_pks.append(
            gaussian(x, *params_temp)
        )  # Append modeled peak y-values to list
        j += num_fun_params

    y_fun_pks = np.asarray(
        y_fun_pks
    )  # convert list to an array, generally takes less memory
