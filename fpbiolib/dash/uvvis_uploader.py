...
datatype_import_selector_modal = dbc.Modal([
    ...
    dbc.ModalBody(
        [
            ...
            html.Div([
                html.Div([
                    html.H5("Single X Column, One or More Y"),
                    dbc.CardImg(src="assets/x_many_y_values.jpg", title="X with Many Y Values", class_name='data_modal'),
                    dbc.Button("One X Many Y", color="primary", id="x_many_y_button"),
                    ],
                    className='import_col_1'
                ),
                html.Div([
                    html.H5("Multiple X, Y Column Pairs"),
                    dbc.CardImg(src="assets/many_x_y.jpg", title="Many X,Y pairs", class_name='data_modal'),
                    dbc.Button("Many X, Y Pairs", color="primary", id="many_x_y_button"),
                    ],
                    className='import_col_2'
                ),
                html.Div([
                    html.H5("Beckman DU-800 exported csv file"),
                    dbc.CardImg(src="assets/beckman_uv_csv.jpg", title="Beckman csv", class_name='data_modal'),
                    dbc.Button("Beckman csv", color="primary", id="beckman_csv_button"),
                    ],
                    className='import_col_1'
                ),
                html.Div([
                    html.H5("Beckman DU-800 dux file (wavelength scan files only)"),
                    dbc.CardImg(src="assets/beckman_instrument.jpg", title="Beckman dux", class_name='data_modal'),
                    dbc.Button("Beckman dux", color="primary", id="beckman_dux_button"),
                    ],
                    className='import_col_2'
                ),                                              
                ], 
                className='importer_container',
            ),
            ]
        ),                        
    ],
...
def uploader_layout(id):
    ...
    dcc.Upload([
        ...
        'Drag and Drop or ', 
        html.A('Select Files'), 
        html.P('(.csv or single-sheet .xlsx)', 
        className='upload_note'),
...
def uploader_callbacks(app, id):
    ...
    @app.callback(
        ...
        Input("beckman_csv_button", "n_clicks"),
        Input("beckman_dux_button", "n_clicks"),
        State("main_graph_data_import_modal", "is_open"), 
    )
    def show_modal(filename, n_x_many_y, n_x_y, n_beckman, n_dux, is_open):
        """Show modal for adding a label."""
        # Open the modal when file is uploaded
        if filename is not None:
            return not is_open
        # Close the modal when the datatype is selected
        elif n_x_many_y or n_x_y or n_beckman or n_dux:
            return is_open
    ...
    def parse_trace_df(n1, n2, n3, n4, session_id, contents, filenames):
        ...
        if contents is None:
            ...
            return None, html.Div('No file', style={'color':'red'}), no_update, no_update, ''

        uploaded_trace_filenames, df, import_status, x_min, x_max = parse_uploaded_traces(contents, filenames, changed_id)
        ...
        write_numeric(340.0, f'ref_lambda_{session_id}')
...
def simple_uploader_callbacks(app):
    ...
    def update_output(list_of_contents, list_of_names):
      if list_of_contents is not None:
          ...
          df.to_csv('exports/simple_dash_upload_df.csv')  # Uncomment to write csv to local storage
