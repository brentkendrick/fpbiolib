...
def du_800_file_converter(fname):
    ...
    sample_name_start_positions = [
        m.end() + 1 for m in re.finditer(sample_name_start_hex_string, s)
    ]
    ...
    wavelength_start_positions = [
        m.start() + 44 for m in re.finditer(data_start_hex_string, s)
    ]
...
def parse_du800_csv_data(contents, filename):
    ...
    try:
        ...
        df = pd.read_csv(io.StringIO(decoded.decode("utf-8")), names=col_names)
    except Exception as e:
        print(e)
        raise
